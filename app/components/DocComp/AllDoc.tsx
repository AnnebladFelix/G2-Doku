  ...
  const fetchInitialData = useCallback(async () => {
    try {
      if (session?.user?.sub && status === "authenticated") {
        const [documentsResponse, flaggedDocumentsResponse] = await Promise.all(
          [
            axios.get("/api/docs"),
            axios.get(`/api/flags/${session?.user?.sub}`).catch((error) => {
              console.error(
              ...
              {sortedDocuments.map((document) => (
                <Table.Row key={document.id}>
                  ...
                  <Table.Cell>
                    <button
                      onClick={() =>
                        handleFlagClick(document.id, session?.user?.sub)
                      }
                    >
